...
class Hunters(QWidget):
    ...
    def initHunterSearch(self):
        ...
        self.layout.addWidget(self.SearchBox,2,0,1,2)
    ...
    def updateTopKilled(self):
        ...
        if sameTeamCount > -1:
            self.TopKilledBox.layout.addWidget(QLabel("You've been on their team %d times." % sameTeamCount))
    ...
    def updateTopKiller(self):
        ...
        sameTeamCount = self.SameTeamCount(data)
        if sameTeamCount > -1:
            self.TopKillerBox.layout.addWidget(QLabel("You've been on their team %d times." % sameTeamCount))
    ...
    def ShowResults(self,data, name):
        ...
        if len(data) <= 0:
            results.layout.addWidget(QLabel("You've never encountered %s in a Hunt." % name))
        else:
            pid = data[0]['profileid']
            results.layout.addWidget(QLabel("You've seen %s in %d hunts." % (name,len(data))))

            allnamesarray = execute_query("select blood_line_name from 'hunters' where profileid is %d group by blood_line_name" % pid)
            allnames = []
            ...
            if len(allnames) > 1:
                results.layout.addWidget(QLabel("They've also gone by:\n%s" % ",".join(allnames)))
            if sameTeamCount > 0:
                results.layout.addWidget(QLabel("You've been on their team %d times." % sameTeamCount))
            if killedby > 0:
                results.layout.addWidget(QLabel("They've killed you %d times." % killedby))
            if killed > 0:
                results.layout.addWidget(QLabel("You've killed them %d times." % killed))
    ...
    def SameTeamCount(self,data):
        ...
        teams = []
        for id in teamnums:
            t = execute_query("select blood_line_name from 'hunters' where game_id is '%s' and team_num is %d" % (id,teamnums[id]))
            team = []
            for n in t:
                team.append(n[0])
            teams.append(team)
    ...
    def updateFrequentHunters(self):
        ...
        for hunter in hunters:
            ...
            if sameTeamCount > 0:
                hWidget.layout.addWidget(QLabel("You've been on their team %d times." % sameTeamCount),1,1)
            killText = []
            if killedme > 0:
