...
def write_json_to_db(file):
    timestamp = file.split("attributes_")[1].split('json')[0]
    with open(file,'r') as f:
        data = json.load(f)

    data = clean_data(translateJson(data,file))
    ...
    for n in teams:
        insert_row("team",teams[n],timestamp)
    for n in entries:
        insert_row("entry",entries[n],timestamp)
    for n in hunters:
        insert_row("hunter",hunters[n],timestamp)
    insert_row("game",game,timestamp)

def insert_row(table,row,timestamp):
    ...
    try:
        ...
        if 'has no column' in str(msg):
            ...
            row.pop(column)
            insert_row(table,row,timestamp)
