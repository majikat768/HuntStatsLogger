...
class KdaWidget(QWidget):
    ...
    def update(self):
        ...
        aData = execute_query("select amount from (select amount, 'entries_view'.timestamp as ts, MissionBagIsQuickPlay from 'entries_view' join 'games_view' on 'games_view'.game_id = 'entries_view'.game_id where category is 'accolade_players_killed_assist') %s" % (condition))
